...
import openlr.mapviewer.maplayer.PoiAccessMapLayer;
import openlr.mapviewer.utils.Formatter;
import openlr.mapviewer.utils.LineNameResolver;

/**
 * The Class LocationTypePoiWithAccess.
 */
public class LocationTypePoiWithAccess extends AbstractLocationType {
    ...
    public final Location createLocation(final String identifier)
        ...
        if (line != null && poi != null) {
            Location loc = LocationFactory
                    .createPoiAccessLocationWithSideAndOrientation(
                            identifier, line, poff,
                            poi.getLongitudeDeg(), poi.getLatitudeDeg(),
                            sideOfRoad, orientation);
            return loc;
        } else {
            String missing;
            if (line == null) {
	...
	public final List<String> getLocationInfo() {
		...
		if (line != null) {
			...
			if (poi != null) {
				info.add("geo coord: "
						+ Formatter.COORD_FORMATTER.format(poi
								.getLongitudeDeg())
						+ "/"
	...
	public final MapLayer getLocationMapLayer() {
		return new PoiAccessMapLayer(line, poffCoord, poi);
	}
	...
	public final void setLocation(final Location loc)
		...
		if (loc.getLocationType() == LocationType.POI_WITH_ACCESS_POINT) {
			line = loc.getPoiLine();
    ...
    public final LocationType getLocationType() {
        return LocationType.POI_WITH_ACCESS_POINT;
    }
}
