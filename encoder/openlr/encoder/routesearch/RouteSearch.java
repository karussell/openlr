...
public class RouteSearch {
	 ...
	 */
	public final RouteSearchResult calculateRoute(final FunctionalRoadClass lfrc)
			throws OpenLREncoderProcessingException {
		if (LOG.isDebugEnabled()) {
		...
		if (startLine.getID() == destLine.getID() && location.size() > 1) {
			...
			while (!data.isOpenEmpty()) {
				...
				// end
				extractNextLines(actualElement, data, lfrc);
			}
			if (result == null) {
				//no route found
	...
	private void extractNextLines(final PQElem actualElement,
			final RouteSearchData data, final FunctionalRoadClass lfrc) {
		Iterator<? extends Line> iterNext = actualElement.getLine()
				.getNextLines();
		// check all successors
		while (iterNext.hasNext()) {
			Line succ = iterNext.next();
			if (FRC_COMPARATOR.compare(succ.getFRC(), lfrc) < 0) {
				continue;
			}
