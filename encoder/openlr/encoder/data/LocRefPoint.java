...
public class LocRefPoint implements LocationReferencePoint {
	...
	public LocRefPoint(final List<Line> route, final OpenLREncoderProperties p)
		...
		isPointOnLine = false;
		isLast = false;
		longitude = line.getStartNode().getLongitudeDeg();
		latitude = line.getStartNode().getLatitudeDeg();
		bearing = calculateBearing(p);
	}

	/**
	 * Instantiates a new location reference point based on another LRP and
	 * assigns a given route to it. Used to correct the route for the last
	 * intermediate LRP of closed lines.
	 * 
	 * @param route
	 *            the shortest-path to the next LRP
	 * @param lrp
	 *            the lrp to be copied (except for the route)
	 * @throws OpenLRProcessingException
	 *             the open lr processing exception
	 */
	public LocRefPoint(final List<Line> route, final LocRefPoint lrp)
			throws OpenLRProcessingException {
		line = lrp.line;
		subroute = route;
		isPointOnLine = lrp.isLRPOnLine();
		isLast = lrp.isLastLRP();
		longitude = lrp.getLongitudeDeg();
		latitude = lrp.getLatitudeDeg();
		bearing = lrp.getBearing();
		next = lrp.next;
		sequenceNumber = lrp.getSequenceNumber();
	}

	/**
	 * Instantiates a new loc ref point.
	 * 
	 * @param coord
	 *            the coord
	 */
	public LocRefPoint(final GeoCoordinates coord) {
		line = null;
		subroute = null;
		isPointOnLine = false;
		isLast = false;
	...
	/**
	 * Instantiates a new location reference point based on a node and this LRP
	 * is the last LRP in the sequence of the LRPs.
	 * 
	 * @param l
	 *            the referenced line
	 * @param p
	...
	public LocRefPoint(final Line l, final OpenLREncoderProperties p)
		...
		isPointOnLine = false;
		isLast = true;
		longitude = l.getEndNode().getLongitudeDeg();
		latitude = l.getEndNode().getLatitudeDeg();
		bearing = calculateBearing(p);
	}

	/**
	 * Instantiates a new location reference point based on a point directly on
	 * a line. This LRP is not based on a node and it is not the last LRP in the
	 * sequence of LRPs.
	 *
